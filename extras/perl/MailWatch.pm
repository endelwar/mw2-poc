...
my $server_port = 51038;
my $timeout = 3600;


# Modify this as necessary for your configuration
my($db_name) = 'mailwatch2';
my($db_host) = 'localhost';
my($db_user) = 'mailwatch2';
my($db_pass) = 'mailwatch2';
 ...
 sub InitMailWatchLogging {
   ...
   if ($pid) {
     ...
     waitpid $pid, 0;
     MailScanner::Log::InfoLog("Started MailWatch2 SQL Logging child");
   } else {
     # New process
 ...
 sub ListenForMessages {
   ...
   while (my $cli = accept(CLIENT, SERVER)) {
    ...
    if (!$sth) {
     MailScanner::Log::WarnLog("$$message{id}: MailWatch2 SQL Cannot insert row: %s", $sth->errstr);
    } else {
     MailScanner::Log::InfoLog("$$message{id}: Logged to MailWatch2 SQL");
    }
